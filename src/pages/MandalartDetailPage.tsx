...
const MandalartDetailPage: React.FC = () => {
  ...
  const [loadingObjectiveAI, setLoadingObjectiveAI] = useState<number | null>(
    null
  );
  ...
  const handleSubjectAIUpdateAndSave = useCallback(
    async (
      subjectId: string,
      newSubjectName: string,
      currentMandalart: MandalartData
    ) => {
      setLoadingSubjectAI(true);
      try {
        await mandalartService.updateSubject(
          subjectId,
          newSubjectName,
          "IN_PROGRESS"
        );

        const aiResult = await mandalartService.generateGeminiSubject(
          newSubjectName
        );
        ...
        if (
          ...
          showSnackbar(
            "AI 제안을 생성하지 못했습니다. AI 응답이 비어있거나 유효하지 않습니다.",
            "warning"
          );
        ...
        setMandalart((prev: MandalartData | null) => {
          ...
          newMandalart.objectives = newMandalart.objectives.map(
            (objective: MandalartItem, index: number) => ({
              ...objective,
          ...
          newMandalart.actions = newMandalart.actions.map(
            (actionList: MandalartItem[]) =>
              actionList.map((action: MandalartItem) => ({
                ...action,
                status: "IN_PROGRESS",
              }))
          );
          newMandalart.actions.forEach(
            (actionList: MandalartItem[], objIndex: number) => {
              actionList.forEach((action: MandalartItem, actIndex: number) => {
                action.name =
                  aiResult.actions[objIndex]?.[actIndex] || action.name;
              });
            }
          );
        ...
        showSnackbar(
          "주제와 연결된 모든 항목이 새로 제안되었습니다.",
          "success"
        );

        const updatePromises: Promise<void>[] = [];
        currentMandalart.objectives.forEach(
          (objective: MandalartItem, index: number) => {
            const newObjectiveName = aiResult.objectives[index];
            if (newObjectiveName) {
              updatePromises.push(
                mandalartService.updateObjective(
                  String(objective.id),
                  newObjectiveName,
                  "IN_PROGRESS"
                )
        ...
        currentMandalart.actions.forEach(
          (actionList: MandalartItem[], objIndex: number) => {
            actionList.forEach((action: MandalartItem, actIndex: number) => {
              const newActionName = aiResult.actions[objIndex]?.[actIndex];
              if (newActionName) {
                updatePromises.push(
                  mandalartService.updateAction(
                    String(action.id),
                    newActionName,
                    "IN_PROGRESS"
                  )
  ...
  const handleObjectiveAIUpdateAndSave = useCallback(
    async (
      objectiveId: string,
      ...
      try {
        await mandalartService.updateObjective(
          objectiveId,
          newObjectiveName,
          "IN_PROGRESS"
        );

        const aiResult = await mandalartService.generateGeminiObjective(
          newObjectiveName
        );
        ...
        setMandalart((prev: MandalartData | null) => {
          ...
          newMandalart.actions[objectiveIndex] = newMandalart.actions[
            objectiveIndex
          ].map((action: MandalartItem, index: number) => ({
            ...action,
        ...
        console.error(
          `AI generation for objective ${objectiveIndex} failed:`,
          _error
        );
        showSnackbar("AI 업데이트에 실패했습니다. 다시 시도해주세요.", "error");
        setMandalart(currentMandalart);
  ...
  const updateMandalartItemStatus = useCallback(
    async (itemId: number, itemType: "subject" | "objective" | "action") => {
      ...
      setMandalart((prevMandalart: MandalartData | null) => {
        ...
        const newMandalart: MandalartData = JSON.parse(
          JSON.stringify(prevMandalart)
        );
        ...
        if (itemType === "subject") {
          ...
          newMandalart.actions = newMandalart.actions.map(
            (objActions: MandalartItem[], objIndex: number) => {
              const updatedActions = objActions.map((action) => {
                if (action.id === itemId) {
                  toggledActionObjectiveIndex = objIndex;
                  return {
                    ...action,
                    status: action.status === "DONE" ? "IN_PROGRESS" : "DONE",
                  };
                }
                return action;
              });
          ...
          if (toggledActionObjectiveIndex !== null) {
            const allActionsDone = newMandalart.actions[
              toggledActionObjectiveIndex
            ].every((action: MandalartItem) => action.status === "DONE");

            if (allActionsDone) {
              newMandalart.objectives[toggledActionObjectiveIndex].status =
                "DONE";
            } else {
              newMandalart.objectives[toggledActionObjectiveIndex].status =
                "IN_PROGRESS";
        ...
        const allObjectivesDone = newMandalart.objectives.every(
          (o: MandalartItem) => o.status === "DONE"
        );
      ...
      try {
        ...
        if (itemType === "subject") {
          ...
          await mandalartService.updateAction(
            String(itemId),
            undefined,
            newStatus
          );
  ...
  const handleSubMandalartItemNameChange = useCallback(
    (
      itemId: number,
      itemType: "subject" | "objective" | "action",
      newName: string
    ) => {
      if (!subMandalartData) return;
      ...
      if (itemType === "subject") {
        setSubMandalartData((prev) =>
          prev ? { ...prev, subject: newName } : null
        );
      } else if (itemType === "objective") {
        setSubMandalartData((prev) =>
          prev
                ...
                actions: prev.actions.map((actionList, oIndex) =>
                  oIndex === objIndex
                    ? actionList.map((action, aIndex) =>
                        aIndex === actIndex ? newName : action
                      )
                    : actionList
                ),
              }
  ...
  const updateMandalartItemName = useCallback(
    async (
      ...
      setMandalart((prev: MandalartData | null) => {
        ...
        if (itemType === "subject")
          newMandalart.subject = findAndUpdate(newMandalart.subject);
        if (itemType === "objective")
          newMandalart.objectives = newMandalart.objectives.map(findAndUpdate);
        if (itemType === "action")
          newMandalart.actions = newMandalart.actions.map(
            (objActions: MandalartItem[]) => objActions.map(findAndUpdate)
          );
        return newMandalart;
      });
      ...
      try {
        if (itemType === "subject") {
          await handleSubjectAIUpdateAndSave(
            String(itemId),
            newName,
            originalMandalart
          );
        } else if (itemType === "objective") {
          const objectiveIndex = mandalart.objectives.findIndex(
            (o: MandalartItem) => o.id === itemId
          );
          if (objectiveIndex === -1)
            throw new Error("Could not find objective in state");
          await handleObjectiveAIUpdateAndSave(
            String(itemId),
            newName,
            objectiveIndex,
            originalMandalart
        ...
        setMandalart(originalMandalart);
      } finally {
        setLoadingSubjectAI(false);
        setLoadingObjectiveAI(null);
      }
    ...
    [
      ...
      setMandalart,
      showSnackbar,
      handleSubjectAIUpdateAndSave,
      handleObjectiveAIUpdateAndSave,
    ]
  );
            ...
            {subMandalartStep === 0 && (
              <Box>
                <TextField
                  ...
                  onChange={(e) =>
                    setSubMandalartData((prev) =>
                      prev ? { ...prev, mandalartName: e.target.value } : null
                    )
                  }
                  sx={{ mb: 2 }}
                />
              ...
              <Box>
                <MandalartGrid
                  data={{
                    ...
                    objectives: subMandalartData.objectives.map(
                      (name, index) => ({ id: index, name })
                    ),
                    actions: subMandalartData.actions.map(
                      (actionList, objIndex) =>
                        actionList.map((name, actionIndex) => ({
                          id: objIndex * 100 + actionIndex,
                          name,
                        }))
                    ),
                  }}
          ...
          {subMandalartStep === 0 ? (
            <>
              ...
              <Button
                ...
                disabled={
                  loadingSubMandalartAI ||
                  !subMandalartData?.mandalartName ||
                  !subMandalartData?.subject
                }
              >
                {loadingSubMandalartAI ? (
            ...
            <>
              <Button onClick={() => setSubMandalartStep(0)}>이전</Button>
              <Button
                onClick={() => setOpenSubMandalartDialog(false)}
                disabled={loadingSubMandalartSave}
              >
                취소
              </Button>
              <Button
